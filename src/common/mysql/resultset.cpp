...
bool parseColumn(const Message &m, Column &c)
{
	...
	if (!getLenEncStr(d, len, pos, c.catalog) ||
	    !getLenEncStr(d, len, pos, c.schema) ||
	    !getLenEncStr(d, len, pos, c.table) ||
	    !getLenEncStr(d, len, pos, c.org_table) ||
	    !getLenEncStr(d, len, pos, c.name) ||
	    !getLenEncStr(d, len, pos, c.org_name)) {
		return false;
	}
	...
	pos += 2;
	c.length = (uint32_t)d[pos] | ((uint32_t)d[pos + 1] << 8) |
...
bool parseResultSet(const uint8_t *data, size_t len, uint32_t caps, ResultSet &out)
{
	...
	if (classifyResponse(m.payload.data(), m.payload.size(), caps) !=
