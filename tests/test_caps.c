...
static void probe_arena(void)
{
    ...
    int fd =
        bpf_map_create((enum bpf_map_type)MAP_TYPE_ARENA, "probe_arena", 0, 0, 1 /* page */, &opts);
...
{
    int fd =
        bpf_map_create(BPF_MAP_TYPE_SOCKMAP, "probe_dpipes", sizeof(__u32), sizeof(__u32), 8, NULL);
...
int main(void)
{
    ...
    printf("\nsk_msg helpers:\n");
    probe_helper("bpf_msg_redirect_hash", BPF_PROG_TYPE_SK_MSG, BPF_FUNC_msg_redirect_hash,
                 TIER_REQUIRED);
    probe_helper("bpf_msg_redirect_map", BPF_PROG_TYPE_SK_MSG, BPF_FUNC_msg_redirect_map,
                 TIER_DESIGN);
    probe_helper("bpf_sk_storage_get", BPF_PROG_TYPE_SK_MSG, BPF_FUNC_sk_storage_get, TIER_DESIGN);
    ...
    probe_helper("bpf_msg_pop_data", BPF_PROG_TYPE_SK_MSG, BPF_FUNC_msg_pop_data, TIER_INFO);
    ...
    probe_helper("bpf_sk_redirect_map", BPF_PROG_TYPE_SK_SKB, BPF_FUNC_sk_redirect_map, TIER_INFO);
    probe_helper("bpf_sk_storage_get  [expect NO]", BPF_PROG_TYPE_SK_SKB, BPF_FUNC_sk_storage_get,
                 TIER_INFO);
    probe_helper("bpf_get_socket_cookie", BPF_PROG_TYPE_SK_SKB, BPF_FUNC_get_socket_cookie,
                 TIER_INFO);
    probe_helper("bpf_skb_pull_data", BPF_PROG_TYPE_SK_SKB, BPF_FUNC_skb_pull_data, TIER_INFO);

    printf("\nsockops helpers:\n");
    probe_helper("bpf_sock_hash_update", BPF_PROG_TYPE_SOCK_OPS, BPF_FUNC_sock_hash_update,
                 TIER_REQUIRED);
    probe_helper("bpf_sock_map_update", BPF_PROG_TYPE_SOCK_OPS, BPF_FUNC_sock_map_update,
                 TIER_DESIGN);
    ...
    probe_helper("bpf_get_netns_cookie", BPF_PROG_TYPE_SOCK_OPS, BPF_FUNC_get_netns_cookie,
                 TIER_INFO);

    printf("\ntimers:\n");
    probe_helper("bpf_timer_init (sched_cls)", BPF_PROG_TYPE_SCHED_CLS, BPF_FUNC_timer_init,
                 TIER_DESIGN);
