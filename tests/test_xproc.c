 ...
 *
 * Run inside the controller as root:
 ...
 * LIBBPF_PIN_BY_NAME and we hand it BNCL_PIN_DIR as the pin root. The child
 * opens them by path, exactly as UCLIENT's loader will. */
#define PIN_CPIPES BNCL_PIN_DIR "/cpipe_map"
#define PIN_INFO BNCL_PIN_DIR "/pipe_sk_info_map"
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--in-proc") == 0) {
            ...
            return run_child(atoi(argv[i + 1]), atoi(argv[i + 2]), atoi(argv[i + 3]));
        } else {
     ...
     * knows its direction. */
    if (bpf_prog_attach(bpf_program__fd(skel->progs.splice_c2d), cpipes_fd, BPF_SK_MSG_VERDICT,
                        0)) {
        fprintf(stderr, "attach client->dpipe failed: %s\n", strerror(errno));
        goto out;
    }
    c2d_on = 1;
    if (bpf_prog_attach(bpf_program__fd(skel->progs.splice_d2c), dpipes_fd, BPF_SK_MSG_VERDICT,
                        0)) {
    ...
    if (use_socketpair) {
        ...
        dp_a = sv[0];
        dp_b = sv[1];
        fprintf(stderr, "note: --socketpair, so the dpipe is AF_UNIX. sk_msg runs\n"
    ...
    if (in_proc) {
        ...
        /* The maps are already pinned under BNCL_PIN_DIR by libbpf; the
        ...
        if (child == 0) {
            ...
            execl("/proc/self/exe", argv[0], "--child", sport, srfd, swfd, (char *)NULL);
            _exit(127);
        }
        close(c2p[1]);
        c2p[1] = -1;
    ...
    }
    {
        ...
        if (poll(&pfd, 1, WAIT_MS) <= 0) {
            fprintf(stderr, "FAIL: the client's write never arrived on the dpipe.\n"
...
out:
    ...
    if (c2d_on) {
        bpf_prog_detach2(bpf_program__fd(skel->progs.splice_c2d), cpipes_fd, BPF_SK_MSG_VERDICT);
    }
    if (d2c_on) {
        bpf_prog_detach2(bpf_program__fd(skel->progs.splice_d2c), dpipes_fd, BPF_SK_MSG_VERDICT);
    }
    if (c2p[0] >= 0) {
        close(c2p[0]);
    }
